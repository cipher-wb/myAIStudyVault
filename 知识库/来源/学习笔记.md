---
type: source
title: "学习笔记"
source_type: course-notes
author: 熠辉（Vibe Coding 实战课）
date_published:
url: "https://www.bilibili.com/cheese/play/ep2410438"
created: 2026-04-23
updated: 2026-04-25
confidence: high
tags:
  - source/course-notes
  - domain/web-dev
  - domain/git
  - domain/tools
  - domain/bot
status: developing
related:
  - "[[Git]]"
  - "[[GitHub]]"
  - "[[VS Code]]"
  - "[[React]]"
  - "[[Next.js]]"
  - "[[shadcn/ui]]"
  - "[[Vercel]]"
  - "[[OpenClaw]]"
  - "[[Bewildcard]]"
  - "[[GPT-Image-2]]"
  - "[[Git 版本控制]]"
  - "[[Vibe Coding 工具链]]"
  - "[[Next.js 项目结构]]"
  - "[[React 组件]]"
  - "[[Next.js API Routes]]"
  - "[[OpenClaw 命令速查]]"
  - "[[API 代充与跨境支付]]"
sources:
  - "[[.源文档/文字稿/学习笔记.md]]"
  - "[[.源文档/文章/2.1 AI 时代的前后端开发核心概念与最佳实践.md]]"
key_claims:
  - "Git 是编程中控制版本的最重要的工具，是 AI 更改的后悔药"
  - "VS Code + Claude Code 插件 ≈ Cursor，可替代使用"
  - "shadcn/ui 可用于参考 UI 组件动态效果，帮助向 AI 描述需求"
  - "Next.js 是建立在 React 基础上的全栈框架"
  - "App Router 文件结构直接决定 URL 路径"
  - "OpenClaw 是 Bot 网关管理工具，支持 QQ Bot 等通道"
  - "Bewildcard 解决中国用户 API 信用卡充值问题"
  - "gptsapi-image-generator skill 可调用 GPT-Image-2"
---

# 学习笔记

> **来源**：【熠辉】面向新手的 Vibe Coding 实战课（Bilibili）+ 日常补充
> **原件**：[[.源文档/文字稿/学习笔记.md]]

## TL;DR

从零开始的全栈开发学习笔记，覆盖工具链、Git 版本控制、Next.js 全栈开发、Bot 网关管理和 API 服务使用。

---

## 1. 工具链概览

| 工具 | 用途 | 备注 |
|---|---|---|
| [[VS Code]] + Claude Code 插件 | 代码编辑 + AI 编程 | ≈ Cursor，可替代 |
| Live Server | 本地预览 HTML | VS Code 插件 |
| [[shadcn/ui]] | UI 组件参考 | 不知如何描述动态效果时查阅 |
| Vercel 模板 | 网页设计模板 | https://vercel.com/templates/next.js |
| Local Images Plus | Obsidian 外链图片本地化 | 配合 Web Clipper 使用 |
| [[GitLens]] | VS Code Git 可视化 | 查看 commit graph、回退版本 |

---

## 2. Git 版本控制

详见 [[Git 版本控制]]。

### 核心命令速查

```bash
git init                  # 初始化仓库
git clone <url>           # 克隆远程仓库
git add .                 # 添加所有改动到暂存区
git commit -m "说明"      # 提交
git push origin main      # 推送到远程
git pull                  # 拉取远程更新
git status                # 查看当前状态
git branch <name>         # 创建分支
git switch <name>         # 切换分支
git merge <branch>        # 合并分支
git reset <commit-id>     # 回退到某次提交
git push --force          # 强制推送（覆盖远程历史，危险！）
```

### Git vs GitHub

- **[[Git]]** = 本地版本控制系统（完全离线、追踪文件变化）
- **[[GitHub]]** = 基于 Git 的云端托管服务（远程仓库、PR、Issues、Actions）

### 版本回退

- **UI 方式**：安装 [[GitLens]] → Open in Commit Graph → Reset Current Branch to Commit
- **命令行**：`git reset <commit-id>` + `git push --force`（⚠️ 会丢失中间所有提交）

---

## 3. Next.js 全栈开发

详见 [[Next.js 项目结构]]、[[React 组件]]、[[Next.js API Routes]]。

### 框架选型

- **[[React]]** = 引擎（组件系统 + 状态管理）
- **[[Next.js]]** = 完整汽车（引擎 + 车身 + 轮子 + 方向盘）
- 文件系统路由：`app/` 目录结构直接决定 URL 路径
- 服务端组件（默认）vs 客户端组件（`'use client'`）

### 快速开始

```bash
npx create-next-app@latest
npm run dev  # → http://localhost:3000
```

### 部署

[[Vercel]] 免费部署，新手零成本上线。

---

## 4. OpenClaw Bot 网关

详见 [[OpenClaw]] 和 [[OpenClaw 命令速查]]。

- **Web Dashboard**：`http://127.0.0.1:18789/`
- **核心操作**：`gateway start/stop/restart`、`channels add/remove`、`devices approve`
- 支持 QQ Bot 等通道

---

## 5. API 服务与图像生成

详见 [[API 代充与跨境支付]]、[[GPT-Image-2]]。

- [[Bewildcard]]（bewildcard.com）为中国用户提供 API 代理充值
- 已创建 `gptsapi-image-generator` skill，调用 [[GPT-Image-2]] 进行图像生成

> [!gap] GPT-Image-2 API 详细调用方式和 skill 构建方法待学习

---

## 关键资源链接

| 资源 | URL |
|---|---|
| Vibe Coding 实战课 | https://www.bilibili.com/cheese/play/ep2410438 |
| shadcn/ui 组件库 | https://ui.shadcn.com/docs/components |
| Vercel 模板 | https://vercel.com/templates/next.js |
| GitHub SSH 配置教程 | https://www.bilibili.com/video/BV1dV411G77N |
| Bewildcard 代充 | https://bewildcard.com/service |
